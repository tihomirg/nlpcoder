...
public class MainAll {
	...
	public static void main(String[] args) {
		...
		ScorerPipeline scorer = new ScorerPipeline(new RichDeclarationScorer[]{new HitWeightScorer(), new DeclProbScorer(0.4)});
		...
		printMsgAndSetTime("Loading time",time);
				...
				for (Sentence sentence : sentences) {
					...
					if (SYNTHESIS) {
						...
						if (!withConnections.isEmpty()){
							...
							merge.run();
							print(mergeAndSort(merge.getCompletedResult(), completed));
						} else {
	...
	private static PriorityQueue<PartialExpression> sort(List<PartialExpression> completed) {
		PriorityQueue<PartialExpression> pq = new PriorityQueue<PartialExpression>(100, new PartialExpressionComparatorDesc());
		pq.addAll(completed);
		return pq;
	}

	private static void print(PriorityQueue<PartialExpression> pq) {
		while(!pq.isEmpty()){
